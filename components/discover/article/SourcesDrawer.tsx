...
export function SourcesDrawer({ isOpen, onClose, sectionTitle, citations }: SourcesDrawerProps) {
  // Flatten all sources from citations
  const allSources: SourceWithArticle[] = citations.flatMap(c => [
    { ...c.primarySource, title: c.primarySource.title || c.primarySource.name },
    ...c.additionalSources.map(s => ({ ...s, title: s.title || s.name })),
  ]);
...
function SourceItem({ source }: { source: SourceWithArticle }) {
  const domain = getDomain(source.url);
  
  // Generate a descriptive title from the URL if not provided
