...
import { Sun, CloudSun, Cloud, CloudRain, MapPin, Loader2, ChevronDown, X, Search } from 'lucide-react';
...
export function WeatherWidget() {
  ...
  return (
    <div className="flex flex-col gap-3 relative">
      {/* Location header with click to change */}
      <button 
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          <span className="truncate max-w-[160px]">{weather.location}</span>
      ...
      {isModalOpen && (
        ...
        >
          ...
          <div className="flex items-center justify-between p-3 border-b border-os-border-dark">
            <span className="text-sm font-medium text-brand-vanilla">Change Location</span>
            <button 
          ...
          {searchResults.length > 0 && (
            <div className="max-h-[200px] overflow-y-auto border-b border-os-border-dark">
              {searchResults.map((city, index) => (
                <button
                  key={`${city.lat}-${city.lon}-${index}`}
                  onClick={() => handleLocationSelect(city)}
                  className="w-full px-3 py-2.5 text-left text-sm text-brand-vanilla hover:bg-os-bg-dark transition-colors flex items-center gap-2"
                >
                  <MapPin className="w-4 h-4 text-os-text-secondary-dark shrink-0" />
          ...
          {searchResults.length === 0 && !locationInput && (
            <div className="p-3 border-b border-os-border-dark">
              <div className="text-xs text-os-text-secondary-dark uppercase tracking-wider mb-2">Popular Cities</div>
          ...
          <div className="p-3">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-brand-aperol/10 text-brand-aperol rounded-lg text-sm font-medium hover:bg-brand-aperol/20 transition-colors"
            >
